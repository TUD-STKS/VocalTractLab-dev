...
//  int i;
  ...
  {
      wxString label = data->gesturalScore.getGestures()[i].name;
...
void GesturalScorePage::lengthenGesturalScore()
{
  ...
  GesturalScoreWithHistory *gs = &data->gesturalScore;

  for (i=0; i < GesturalScore::NUM_GESTURE_TYPES; i++)
  {
    index = gs->getGestures()[i].getIndexAt(pos_s);
    if (index != -1)
    {
      gs->getGestures()[i].getGesture(index)->duration_s+= DELTA_LENGTH_S;
    }
  }
...
void GesturalScorePage::OnSynthesize(wxCommandEvent &event)
{
  ...
  TubeSequence *tubeSequence = &*data->gesturalScore.get();
